...
class Preferences:
    def __init__(self, prefer_small_gene_names=None, prefer_first_selection_on_multiple=None, prefer_remember_selection=None, prefer_output=None):
...
def retrieve_gene_name_facilitator(input_term, input_filenames, preferences):
    try:
        ...
        log = Log()
        if len(results[0]) == 1: # if there is 1 gene name given
